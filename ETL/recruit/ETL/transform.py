...
def json_to_dataframe(**context):
    """
        GCS에 적재된 rawdata(json)를 전처리 후 dataframe으로 변환
        return : dataframe
    """
    ti = context['task_instance']
    data = ti.xcom_pull(key="return_value", task_ids='pull_rawdata')

    jobs = data['rawdata']

    postings = []
    for job in jobs:
        postings.append(preprocessing(job))
