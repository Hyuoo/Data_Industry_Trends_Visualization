...
- 인스턴스 다운(서버 측면)에서는 세부적으로 CPU 사용량 / 남은 디스크 용량 / 네트워크 연결 여부(헬스체크) / 활성 프로세스 수 의 크게 4가지 지표를 확인
- Airflow 로그가 쌓여서 서버가 다운된 이력이 있었고, 네트워크가 멈추면서 네트워크와 연결된 알림 기능들이 멈추는 것을 확인하고 좀 더 다양한 전략이 필요하다고 판단함.
